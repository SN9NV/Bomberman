...
cge::GLSLProgram::GLSLProgram(const std::string &vertexFilePath, const std::string &fragmentFilePath) :
		_vertexShaderID(0),
		_fragmentShaderID(0),
		_programID(0),
		_attributeCount(0),
...
bool cge::GLSLProgram::compileShaders(const std::string &vertexFilePath, const std::string &fragmentFilePath) {
...
bool cge::GLSLProgram::linkProgram() {
	// Link the program
...
void cge::GLSLProgram::bindAttribute(const std::string &attributeName) {
	glBindAttribLocation(this->_programID, this->_attributeCount++, attributeName.c_str());
}

void cge::GLSLProgram::start() {
...
GLint cge::GLSLProgram::getUniformLocation(const std::string &uniformName) const {
	if (!this->_isInUse) {
...
void cge::GLSLProgram::uploadFloat(GLint location, float value) const {
	if (!this->_isInUse) {
		std::cerr << "Program is not in use\n";
	}
...
void cge::GLSLProgram::uploadvec3d(GLint location, const glm::vec3 &value) const {
	if (!this->_isInUse) {
		std::cerr << "Program is not in use\n";
	}
...
void cge::GLSLProgram::uploadBool(GLint location, bool value) const {
	if (!this->_isInUse) {
		std::cerr << "Program is not in use\n";
	}
...
void cge::GLSLProgram::uploadMatrix4f(GLint location, const glm::mat4 &value) const {
	if (!this->_isInUse) {
		std::cerr << "Program is not in use\n";
	}
...
bool cge::GLSLProgram::_compileShader(const std::string &shaderSRC, GLuint shaderID) {
...
void cge::GLSLProgram::upload1i(GLint location, GLint value) const {
	if (!this->_isInUse) {
		std::cerr << "Program is not in use\n";
	}

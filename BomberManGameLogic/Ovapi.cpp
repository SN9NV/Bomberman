...
Ovapi::Ovapi(const glm::vec3 &position, const glm::vec3 &rotation, float scale, cge::Model &model, cge::Loader &loader, const Player& player, std::vector<std::vector<cge::Entity *>> &level, float hitBox) :
...
bool Ovapi::update(const cge::InputManager &input, cge::GLSLProgram &shader, unsigned lastFrameTime) {
	...
	/*if (_n_moveDir.x == 0 && _n_moveDir.z == 0) {
	...
	else*/ if ((fmod(_position.x, 1) > 1 - _speed || fmod(_position.x, 1) < _speed && pZ == roundf(this->getPosition().z)) && (_n_moveDir != glm::vec3({0,0,0}))) {
		_n_moveDir.z = 0;
		...
		if (pX > _position.x)
			...
			_n_moveDir.x = -1;
	} else  if (fmod(_position.z, 1) > 1 - _speed || fmod(_position.z, 1) < _speed && pX == roundf(this->getPosition().x)) {
		_n_moveDir.x = 0;

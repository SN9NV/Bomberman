...
bool Ovapi::update(const cge::InputManager &input, cge::GLSLProgram &shader, unsigned lastFrameTime) {
	int moveDir;
	float pX, pZ;
	pX = roundf(_player.getPosition().x);
	pZ = roundf(_player.getPosition().z);
	float x;
	float z;
	static const int timeToRandom = 1000;
	if (((fmod(_position.x, 1) > 1 - _speed || fmod(_position.x, 1) < _speed)
		&& floor(_player.getPosition().z) == floor(this->getPosition().z)
		&& fabs(_player.getPosition().x - _position.x) < 6)
		&& (_n_moveDir != glm::vec3({0, 0, 0}))) {
		_n_moveDir.z = 0;
		...
		if (pX > _position.x)
			...
			_n_moveDir.x = -1;
	} else if (((fmod(_position.z, 1) > 1 - _speed || fmod(_position.z, 1) < _speed)
				&& floor(_player.getPosition().x) == floor(this->getPosition().x)
				&& fabs(_player.getPosition().x - _position.x) < 6)
				&& (_n_moveDir != glm::vec3({0, 0, 0}))) {
		_n_moveDir.x = 0;
